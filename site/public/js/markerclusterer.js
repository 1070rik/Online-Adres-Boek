 ...
 */
 /**
 * Modifications copyright (C) 2017 <Ian Vink>
 **/
/**
...
function MarkerClusterer(map, opt_markers, opt_options) {
    ...
    this.colors = ["%23FFAA00", "%23FFFF00", "%230000FF", "%23AA00FF", "%23FF00FF"];
    ...
    google.maps.event.addListener(this.map_, 'zoom_changed', function() {
		...
		if (infowindow.markerObj === undefined) {infowindow.close();}
...
MarkerClusterer.prototype.setupStyles_ = function() {
    ...
    for (var i = 0, color; color = this.colors[i]; i++) {
        this.styles_.push({
            url: this.imagePath_ + "?color="+color+"&dot=false",
            height: 34,
            width: 22,
			anchor: [22]
        });
...
MarkerClusterer.prototype.calculator_ = function(markers, numStyles) {
    var index = 0;
    var count = 0;
	for(var markNum = 0; markNum < markers.length; markNum++) {
		if(markers[markNum].label == "") {
			count += 1;
		} else {
			count += parseInt(markers[markNum].label);
		}
	}
    var dv = count;
    while (dv !== 0) {
...
ClusterIcon.prototype.onAdd = function() {
	...
	google.maps.event.addDomListener(this.div_, 'click', function() {
		map.panTo(that.center_);
		if (infowindow.markerObj !== undefined) {
...
ClusterIcon.prototype.createCss = function(pos) {
    ...
    if (typeof this.anchor_ === 'object') {
        if (typeof this.anchor_[0] === 'number' && this.anchor_[0] > 0 &&
            ...
            style.push('height:' + this.height_ + 'px; line-height:' + this.anchor_[0] + 
				'px;');
        } else {
